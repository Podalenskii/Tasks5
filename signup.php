...
if (isset($_POST['signup'])) {
    ...
    if (($_POST['password2']) == ''){
        $errors[] = 'Ведите пароль второй раз';
    }
    if (($_POST['password1'] != $_POST['password2']){
        $errors[] = 'Веденный пароли не совподают';
    }
    if (empty($errors)){
        ...
        if ($count == 0){
         ...
         if ($count == 0){
            $errors[] = 'Пользователь с таким email уже зарегистрирован, введите другой email';
              ...
              $count = $statement->exec(INSERT INTO users (username, password, email) VALUE ('$username', '$password', '$email'));
         if ($count == 0){
